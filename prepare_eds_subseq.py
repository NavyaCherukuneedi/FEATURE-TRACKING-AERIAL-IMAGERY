...
def generate_subseq(seq_name, start_idx, end_idx, dt):
    ...
    for n_bins in [5]:
        ...
        with h5py.File(str(input_dir / "events_corrected.h5"), "r") as h5f:  # Ensure the file is open
            print(f"First index: {event_idx[0]}, Last index: {event_idx[1]}")
            print(f"x dataset size: {h5f['x'].shape[0]}")
